...
const requests=[];let upstream,rejectTurn=false,rejectResume=false,rejectRename=false;
try{
 ...
 await call('Runtime.enable');await call('Page.enable');await call('Emulation.setDeviceMetricsOverride',{width:1440,height:900,deviceScaleFactor:1,mobile:false});await call('Page.navigate',{url:`http://127.0.0.1:${port}`});await wait('typeof document.querySelector("#login-form")?.onsubmit==="function"');await sleep(300);
 ...
 await js(`document.querySelector('#prompt').value='Keep my draft'`);
 const beforeMenu=requests.length;
 await js(`document.querySelectorAll('#tasks button')[1].dispatchEvent(new MouseEvent('contextmenu',{bubbles:true,cancelable:true,clientX:100,clientY:160}))`);
 assert.equal(await js('document.querySelector("#task-context-menu")?.hidden'),false,'right-click opens task menu');
 await js(`document.querySelector('#context-rename').click();document.querySelector('#task-name').value='Other renamed';document.querySelector('#rename-form').requestSubmit()`);await wait('!document.querySelector("#rename-dialog").open');
 assert.equal(requests.findLast(m=>m.method==='thread/name/set').params.threadId,'other-task');assert.equal(await js('document.querySelector("#task-title").textContent'),'Browser fixture');assert.match(await js('document.querySelectorAll("#tasks button")[1].textContent'),/Other renamed/);assert.ok(!requests.slice(beforeMenu).some(m=>m.method==='thread/resume'),'context rename must not switch or resume tasks');
 assert.equal(await js('document.querySelector("#prompt").value'),'Keep my draft');
 assert.equal(await js('document.activeElement===document.querySelectorAll("#tasks button")[1]'),true,'focus returns to renamed task');
 await js(`document.querySelectorAll('#tasks button')[1].dispatchEvent(new KeyboardEvent('keydown',{key:'F10',shiftKey:true,bubbles:true,cancelable:true}))`);assert.equal(await js('document.querySelector("#task-context-menu").hidden'),false);
 await js(`document.querySelector('#context-rename').dispatchEvent(new KeyboardEvent('keydown',{key:'Escape',bubbles:true,cancelable:true}))`);assert.equal(await js('document.querySelector("#task-context-menu").hidden'),true);
 await js(`document.querySelectorAll('#tasks button')[1].dispatchEvent(new MouseEvent('contextmenu',{bubbles:true,cancelable:true,clientX:innerWidth-1,clientY:innerHeight-1}))`);
 assert.equal(await js(`(()=>{const r=document.querySelector('#task-context-menu').getBoundingClientRect();return r.right<=innerWidth&&r.bottom<=innerHeight})()`),true,'menu fits viewport');
 await js(`document.querySelector('#prompt').dispatchEvent(new PointerEvent('pointerdown',{bubbles:true}))`);
 assert.equal(await js('document.querySelector("#task-context-menu").hidden'),true,'outside pointer dismisses menu');
 await js('document.querySelector("#rename-task").click()');await wait('document.querySelector("#rename-dialog").open');
 await js(`document.querySelector('#task-name').value='A better title';document.querySelector('#rename-form').requestSubmit()`);await wait('!document.querySelector("#rename-dialog").open');
 assert.equal(await js('document.querySelector("#task-title").textContent'),'A better title');assert.match(await js('document.querySelector("#tasks button").textContent'),/A better title/);
 await js('document.querySelector("#refresh").click()');await sleep(150);assert.match(await js('document.querySelector("#tasks button").textContent'),/A better title/);
 rejectRename=true;await js(`document.querySelector('#rename-task').click();document.querySelector('#task-name').value='Rejected title';document.querySelector('#rename-form').requestSubmit()`);await wait('document.querySelector("#rename-error").textContent.includes("Rename unavailable")');assert.equal(await js('document.querySelector("#task-title").textContent'),'A better title');await js('document.querySelector("#rename-cancel").click()');rejectRename=false;
 ...
 await login('user');assert.match(await js('document.querySelector("#identity").textContent'),/user.*Read-only/);for(const sel of ['#composer','#new-task','#stop','#rename-task'])assert.equal(await js(`document.querySelector('${sel}').hidden`),true);
 ...
 upstream.send(JSON.stringify({method:'thread/name/updated',params:{threadId:'fixture-task',threadName:'External rename'}}));await wait('document.querySelector("#task-title").textContent==="External rename"');
 await js(`document.querySelector('#tasks button').dispatchEvent(new MouseEvent('contextmenu',{bubbles:true,cancelable:true}))`);assert.equal(await js('document.querySelector("#task-context-menu").hidden'),true);
 ...
 assert.deepEqual(errors,[]);console.log('PASS Chromium: right-click and keyboard menu/unselected rename/draft and focus preservation/dismissal/bounds; rename/save/cancel/error/read-only/live update; Plan/Normal payload, settings preservation, active-turn lock, new task reset; admin prompt/stop; user read-only selection, forged-write denial, live SSE, pending approval, reload, logout, mobile width; no JS exceptions.');
}finally{
