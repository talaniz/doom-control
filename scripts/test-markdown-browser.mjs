...
try{
 ...
 const markdown='# Rendered heading\n\n**Bold** and *italic* and ~~removed~~\n\n> Quote\n\n- [x] Done\n- Nested\n  - Child\n\n| A | B |\n| --- | --- |\n| One | Two |\n\n```html\n<script>literal</script>\n```\n\n<img src=x onerror=alert(1)>\n\n![remote](https://example.com/pixel.png)';
 for(const type of ['agentMessage','userMessage','plan']){upstream.send(JSON.stringify({method:'item/completed',params:{threadId:'fixture-task',item:{id:'markdown-'+type,type,...(type==='userMessage'?{content:[{type:'text',text:markdown}]}:{text:markdown})}}}));}
 await wait('document.querySelectorAll(".markdown h1").length===3');
 for(const selector of ['strong','em','del','blockquote','ul','table','input[disabled]','pre code'])assert.equal(await js(`document.querySelectorAll('.markdown ${selector}').length>=3`),true,selector);
 assert.equal(await js('document.querySelectorAll(".markdown img,.markdown script").length'),0);
 upstream.send(JSON.stringify({method:'item/agentMessage/delta',params:{threadId:'fixture-task',itemId:'streamed',delta:'**Streaming'}}));await wait('document.querySelector("#conversation").textContent.includes("Streaming")');
 upstream.send(JSON.stringify({method:'item/agentMessage/delta',params:{threadId:'fixture-task',itemId:'streamed',delta:' complete**'}}));await wait('[...document.querySelectorAll(".markdown strong")].some(n=>n.textContent==="Streaming complete")');
 await call('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:true});assert.equal(await js('document.documentElement.scrollWidth<=innerWidth'),true,'Markdown fits mobile');await call('Emulation.clearDeviceMetricsOverride');
 ...
 assert.deepEqual(errors,[]);console.log('PASS Chromium: rich Markdown in user/agent/plan, inert HTML, code, streamed updates, mobile; Plan/Normal payload, settings preservation, active-turn lock, new task reset; admin prompt/stop; user read-only selection, forged-write denial, live SSE, pending approval, reload, logout, mobile width; no JS exceptions.');
}finally{
