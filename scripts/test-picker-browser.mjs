...
try{
 ...
 await js(`document.querySelector('#new-task').click();document.querySelector('#cwd').value='/tmp/';document.querySelector('#cwd').dispatchEvent(new Event('input'));document.querySelector('#prompt').value='$test';document.querySelector('#prompt').focus();document.querySelector('#prompt').dispatchEvent(new Event('input'))`);
 await wait('document.querySelector("#prompt-picker").textContent.includes("test-skill")');await js(`document.querySelector('#prompt').dispatchEvent(new KeyboardEvent('keydown',{key:'Enter',bubbles:true,cancelable:true}))`);
 rejectTurn=true;await js(`document.querySelector('#composer').requestSubmit()`);await wait('document.querySelector("#notice").textContent.includes("Fixture mode unavailable")');const firstInput=requests.findLast(m=>m.method==='turn/start').params.input;
 rejectTurn=false;await js(`document.querySelector('#composer').requestSubmit()`);await wait('!document.querySelector("#stop").hidden');await wait('document.querySelector("#prompt").value===""');assert.deepEqual(requests.findLast(m=>m.method==='turn/start').params.input,firstInput,'normalized cwd retry preserves references');
 upstream.send(JSON.stringify({method:'turn/completed',params:{threadId:'fixture-task',turn:{id:'new-turn',status:'completed'}}}));await wait('document.querySelector("#stop").hidden');await sleep(150);
