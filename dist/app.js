...
function renderItem(i){if(!i?.id)return;if(i.type==='reasoning'&&!itemText(i))return;let row=items.get(i.id);if(!row){row=el('article',undefined,'message '+(i.type==='userMessage'?'user':i.type==='agentMessage'?'agent':'tool'));items.set(i.id,row);$('#conversation .empty')?.remove();$('#conversation').append(row)}const nearBottom=$('#conversation').scrollHeight-$('#conversation').scrollTop-$('#conversation').clientHeight<160;row.replaceChildren();const label=i.type==='userMessage'?'You':i.type==='agentMessage'?'Codex':i.type.replace(/([A-Z])/g,' $1');row.append(el('h3',label));const pre=el('pre');if(['userMessage','agentMessage'].includes(i.type))renderLinkedText(pre,itemText(i));else pre.textContent=itemText(i);if(!['userMessage','agentMessage','plan'].includes(i.type)){const d=el('details');d.append(el('summary',i.status||'Details'),pre);row.append(d)}else row.append(pre);if(nearBottom)$('#conversation').scrollTop=$('#conversation').scrollHeight;row.item=i}
...
function newTask(){if(!canWrite()||starting)return;resetAttachments();taskSettings=null;$('#mode').value='default';loadingTask=false;permissionLabel='Workspace edits · Approve for me';selection++;thread=null;turn=null;items.clear();$('#conversation').replaceChildren(el('div','Start a task with your first prompt.','empty'));title();drawRequests();$('#prompt').focus()}
async function sendPrompt(text){if(!canWrite())throw Error('Read-only account');if(!connected||loadingTask)throw Error('Wait for the selected task to connect.');if(uploading||attachments.some(a=>a.removing))throw Error('Wait for attachment changes to finish');if(!text.trim()&&!attachments.length)throw Error('Enter a prompt or attach a file');if(turn||starting)throw Error('Wait for this turn to finish or stop it first');starting=true;controls();notice('');try{if(!thread){const b=await rpc('thread/start',{cwd:$('#cwd').value,approvalPolicy:'on-request',approvalsReviewer:'auto_review',sandbox:'workspace-write',...($('#model').value?{model:$('#model').value}:{})});taskSettings={model:b.model,reasoning_effort:b.reasoningEffort??null,developer_instructions:null};renderThread(b.thread)}if(!taskSettings?.model)throw Error('Task settings unavailable. Reopen the task and try again.');const b=await rpc('turn/start',{threadId:thread.id,collaborationMode:{mode:$('#mode').value,settings:taskSettings},input:[{type:'text',text:text.trim()?text:'Please review the attached files.'}],approvalPolicy:'on-request',approvalsReviewer:'auto_review'},attachments.map(a=>a.id));resetAttachments(false);turn=b.turn.status==='inProgress'?b.turn.id:null;$('#prompt').value='';await list();return{threadId:thread.id,turnId:b.turn.id}}finally{starting=false;controls()}}
